...
## Grilles postérieures au correctif `rowspan`

Ces grilles reflètent les CSV régénérés après le
[correctif `rowspan`](ameliorations.qmd#correctif-appliqué-rowspan-dans-le-parseur-html).
Les CSV antérieurs restent disponibles sur S3 dans
`reprise/output_csv/marker_pre_rowspan/` pour comparaison.

Les décalages de colonnes qui subsistent chez marker relèvent des autres mécanismes —
remplissage à droite à la lecture, et écart de structure d'en-tête en amont.
:::
...
**Les libellés sont pseudonymisés.** Les variantes d'un même nom dues à l'OCR partagent
le même numéro et se distinguent par un suffixe (`Entité 07` / `Entité 07·b`) : un écart
de libellé entre annotation et prédiction reste donc visible.
:::
